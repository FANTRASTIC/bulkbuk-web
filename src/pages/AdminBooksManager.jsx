...
import { BookOpen, Pencil, Trash2, Plus, Save, X, Search, Archive, RotateCcw } from 'lucide-react';
...
export function AdminBooksManager() {
  const auth = useAuth();
  const { books, addBook, updateBook, deleteBook, archiveBook, restoreBook, PLACEHOLDER_COVER } = useBooks();
  ...
  const filtered = books.filter((b) => {
    ...
    const matchesSearch = !q || [b.title, b.author, b.summary].join(' ').toLowerCase().includes(q);
    return matchesSearch && (showArchived ? b.archived : !b.archived);
  });
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Books Manager</h1>
          <p className="text-muted-foreground mt-1">Create, edit, and manage your book collection</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer">
            <input 
              ...
              className="w-4 h-4"
            />
            <span className="text-sm">Show archived ({books.filter(b => b.archived).length})</span>
          </label>
        </div>
      </div>
      ...
      <Card className="rounded-2xl overflow-hidden">
          ...
          <table className="w-full">
            <thead className="border-b bg-muted/50">
              <tr>
                ...
                <th className="px-6 py-3 text-center text-sm font-semibold">Archive</th>
                <th className="px-6 py-3 text-right text-sm font-semibold">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((book) => (
                <tr key={book.id} className={`border-b hover:bg-muted/30 transition ${book.archived ? 'opacity-60' : ''}`}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <img src={book.coverUrl || PLACEHOLDER_COVER} alt={book.title} className="w-8 h-12 object-cover rounded" onError={(e) => (e.currentTarget.src = PLACEHOLDER_COVER)} />
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex gap-2 justify-end">
                      {!book.archived ? (
                        <>
                          <Button size="sm" variant="outline" onClick={() => { setEditing(book); setShowEditor(true); }}>
                            <Pencil className="w-4 h-4" />
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => handleArchive(book.id)}>
                            <Archive className="w-4 h-4" />
                          </Button>
                          <Button size="sm" variant="destructive" onClick={() => setDeleteConfirm(book)}>
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </>
                      ) : (
                        <>
                          <Button size="sm" variant="outline" onClick={() => handleRestore(book.id)}>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">{showArchived ? 'No archived books' : 'No books found'}</p>
          </div>
        )}
      </Card>
